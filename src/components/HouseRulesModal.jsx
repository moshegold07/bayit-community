...
export default function HouseRulesModal({ onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
      <div style={{ background: "#fff", borderRadius: 12, border: "0.5px solid #e0e0da", padding: "1.5rem", width: "100%", maxWidth: 520, maxHeight: "80vh", display: "flex", flexDirection: "column" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1rem", paddingBottom: "0.75rem", borderBottom: "0.5px solid #eee" }}>
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          {loading
            ...
            : rules
              ? <div style={{ fontSize: 14, lineHeight: 1.8, whiteSpace: "pre-wrap", color: "#333" }}>{rules}</div>
              : <div style={{ color: "#888", fontSize: 14 }}>לא הוגדרו חוקי בית עדיין.</div>
          }
        </div>
        <button onClick={onClose} style={{ marginTop: "1rem", padding: "8px 20px", background: BLUE, color: "#fff", border: "none", borderRadius: 8, fontSize: 14, cursor: "pointer", alignSelf: "flex-end" }}>סגור</button>
      </div>
    </div>
  );
}
